...
function addErrorStyleOnInput (input) {
  addStyles(input, {
...
function newPostCreate(rescaleChangeValue, hashFieldOptions, maxLengthComment, sliderEffectsOptions, linkServer) {
  ...
  function checkerSubmitPostHandler(evt) {
    ...
    if (!checkHashPlaceHandler()) {
      addErrorStyleOnInput(hashTextInput);
      return false;
    }

    if (!checkCommentPlaceHandler()) {
      addErrorStyleOnInput(commentInput);
      return false;
    }
    closeNewPostHandler();
    webRequest(linkServer, [createSuccessMessage], [createErrorMessage],formNewPostCreate);
    return true;
  }
